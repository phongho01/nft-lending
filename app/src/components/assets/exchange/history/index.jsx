...
export default function Table() {
  ...
  const fetchTransactionHistory = async () => {
    try {
      const mintTxs = await getTransactionByEvents(WXDC_ADDRESS, ERC20_ABI, 'Minted');
      const burntTxs = await getTransactionByEvents(WXDC_ADDRESS, ERC20_ABI, 'Burnt');
            ...
            historyTransactions.map((item, index) => (
              <div className={styles['table-list']} key={index}>
                ...
                <div className={styles['table-list-item']}>
                  {formatHexValue(item.amount._hex)} {item.name === 'Minted' ? 'XDC' : 'wXDC'}
                </div>
                <div className={`${styles['table-list-item']} text-center`}>
                  <Icon icon="ep:right" fontSize={16} />
                </div>
                <div className={styles['table-list-item']}>
                  {formatHexValue(item.amount._hex)} {item.name === 'Minted' ? 'wXDC' : 'XDC'}
                </div>
