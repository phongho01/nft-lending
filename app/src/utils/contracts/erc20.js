...
import { WXDC_ADDRESS, LOAN_ADDRESS } from '@src/constants';
...
export const getRawBalance = async (account, contractAddress = WXDC_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
  return contract.balanceOf(account);
};

export const getBalance = async (account, contractAddress = WXDC_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
  const balance = await contract.balanceOf(account);
  return Number(ethers.utils.formatEther(balance)).toFixed(2);
};

export const getContractSymbol = async (contractAddress = WXDC_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
  return contract.symbol();
};

export const checkAllowance = async (owner, amount, spender = LOAN_ADDRESS, contractAddress = WXDC_ADDRESS) => {
  const contract = ERC20Contract(contractAddress, provider);
