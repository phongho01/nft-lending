...
describe("LendingPoolV3", () => {
    beforeEach(async () => {
        ...
        await wXDC.mint(treasury.address, ONE_ETHER.mul(1000));
        ...
        await wXDC.connect(user1).approve(lendingPool.address, MaxUint256);
        await wXDC.connect(user2).approve(lendingPool.address, MaxUint256);
        await wXDC.connect(user3).approve(lendingPool.address, MaxUint256);
        await wXDC.connect(treasury).approve(lendingPool.address, MaxUint256);
    });
    ...
    describe("test share reward", () => {
        it("Should successfully`", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                if (i === 0) {
                    const balanceBefore = await wXDC.balanceOf(treasury.address);
                    await lendingPool.connect(user1).claimReward();
                    const balanceAfter = await wXDC.balanceOf(treasury.address);
