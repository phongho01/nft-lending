...
contract LendingPoolV3 is Permission {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many wXDC tokens the user has provided.
        uint256 rewardDebt; // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of wXDC
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws wXDC tokens to a pool. Here's what happens:
    ...
    // Info of each user that stakes wXDC tokens.
    ...
    constructor(IERC20 _wXDC, address _treasury, uint256 _rewardPerBlock, uint256 _startBlock) {
        wXDC = _wXDC;
        treasury = _treasury;
    ...
    function updatePool() public {
        ...
        uint256 wXDCSupply = poolInfo.stakedSupply;
        if (wXDCSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(REWARDS_PRECISION).div(wXDCSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit wXDC tokens to Lending Pool for Reward allocation.
    function deposit(uint256 _amount) external {
        require(_amount > 0, "amount zero");
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        wXDC.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw staked wXDC tokens from Pool.
    function withdraw(uint256 _amount) external {
        ...
        updatePool();
        wXDC.safeTransfer(address(msg.sender), _amount);
    ...
    function claimReward() public {
        ...
        uint256 _claimable = (_amount * wXDC.balanceOf(treasury)) / _totalPendingReward;
        wXDC.safeTransferFrom(treasury, msg.sender, _claimable);
    ...
    function emergencyWithdraw() external {
        UserInfo storage user = userInfo[msg.sender];
        wXDC.safeTransfer(address(msg.sender), user.amount);
